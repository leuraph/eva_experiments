"""
"This script computes and prints the right-hand side function f
corresponding to the crafted solution
u = 2. * r**(-4/3) * x * y * (1 - x*x) * (1 - y*y),
where r = sqrt(x*x + y*y)
for the problem
-div(A grad u) + phi(u) = f,
where phi(u) = u^3, A = Id.
NOTE: The computations are performed analytically using sympy.
"""
import sympy as sp
