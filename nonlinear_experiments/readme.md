...
1. Compute the reference mesh and a reference solution on it, i.e.
  - generate a graded mesh with the script
   `graded_meshes/refinement_script.m`,
  - and compute the reference solution on the graded mesh
    using the script `graded_solver.py`.
2. Do the experiment(s) `experiments.py`.
3. Compute the energies of the results using the script
   `compute_energies.py`.
4. Plot the energy errors: `plot_energy_errors_combined.py`
...
2. Compute a reference value for $\|u\|_a^2$, i.e.
  - generate a graded mesh using the matlab script
    `graded_meshes/refinement_script.m`. 
  - numerically integrate the term $\|u\|_a^2$
    with `energy_norm_problem_4.ipynb`
    (this is implemented in a Jupyter Notebook
    to enable evaluation on and comparison for different meshes).
   - If needed, change the hard-coded reference value
    for $\|u\|_a^2$ in the script
    `compute_energy_norm_errors_squared_problem-4.py`.
